...
def test_value_roundtrip():
    ...
    assert val.upQualifier == new_val.upQualifier, "Mismatch in upQualifier"  # This may be None
...
def test_protocol_roundtrip():
    ...
    original = mb.Protocol(
        topcategory="TopCat",
        category=mb.EndpointCategory(code="ABC123", term="Some term", title="Some title"),
...
def create_protocolapp4test():
    ...
    return mb.ProtocolApplication(
            ...
            citation=citation,
            effects=[],
            owner=mb.SampleLink.create(sample_uuid="sample-uuid", sample_provider="Sample Provider"),
            protocol=protocol,
...
def test_composition_entry_roundtrip():
    ...
    original = mb.CompositionEntry(
        ...
        proportion=mb.ComponentProportion(
            ...
            real=mb.RealProportion(lowerPrecision="medium", lowerValue=5.0, upperPrecision="high", upperValue=15.0, unit="mg"),
...
def test_composition_roundtrip():
    ...
    original = mb.Composition(
        ...
        feature={"feature_url": {"units" : "g", "name" : "smth"
                                 #, "sameAs" : "mass"
        }}
...
def create_substance_record() -> mb.SubstanceRecord:
    return mb.SubstanceRecord(
        ...
        referenceSubstance=mb.ReferenceSubstance(
            i5uuid="ref-i5uuid",
            uri="http://example.com/reference"
        ),
        study=[
            create_protocolapp4test()
        ],
        composition=None
    )
...
def test_substances_roundtrip():
    ...
    """
    original = mb.Substances(
        substance=[create_substance_record()]
    )
    json_string = original.model_dump_json()
    data = json.loads(json_string)
